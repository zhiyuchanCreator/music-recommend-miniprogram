...
- 💿 **详情（detail）**：专辑详情，支持一键跳转至 QQ音乐 / 网易云音乐 等平台搜索
- 👤 **我的（my）**：个人中心
- 🧩 **自定义底部 Tab（custom-tab-bar）** 与可复用组件（sidebar / skeleton / empty-state）

## 原型设计

本仓库包含一份可查看、可交互的产品原型设计，方便 HR / 面试官快速了解产品结构与交互逻辑：

- **设计文档**：[小程序原型设计.md](./小程序原型设计.md) —— 包含信息架构、用户流程、页面原型、交互说明、组件规范与数据字段。
- **可点击 HTML 原型**：[prototype/index.html](./prototype/index.html) —— 用浏览器打开即可在模拟手机框中预览首页、搜索、详情、我的等页面及弹窗效果。

> 提示：HTML 原型可直接双击打开，无需部署；若需要导入墨刀 / Axure / Figma，可参照设计文档中的结构和尺寸重新绘制。
